...
VARSIZE=$( df -kP /var | grep "/var" | awk '{ print $5 }' | sed 's/%//g' )
if [ $VARSIZE -ge 90 ]; then
  echo_failure "/var utilization > 90%. Clean it up first!"
  exit 1
else
  echo_success "OK"
fi
